...
//struct DDS_Duration_t initSendPeriod = { 1, 0 };
...
static void process_data(
	...
	//FILE *subfp = fopen("subfp1.txt", "w");
	//
    char idChar[127];
    //memcpy(idChar, instance, 10);
	memcpy(idChar, instance, 127);
	//printf("cnt=%d, pixel= %d\n",cnt, *instance);
    idChar[126] = '\0';
	cnt++;
	destarr[cnt] = *instance;
	printf("cnt=%d\t dest arr=%d\n",cnt, destarr[cnt]);
	//fprintf(subfp, "%d", destarr[cnt]);
...
static void on_data_available(
    ...
    if (rc == DDS_RETCODE_NO_DATA) {
        ...
        fprintf(stderr, "! Unable to take data from data reader, "
                        "error %d\n", rc);
		printf("unable to detect data reader");
        return;
    }
	int sub=0;
	...
	//printf("Size of the given dest array is %d\n", sizeof(destarr) / sizeof(unsigned char));
	...
	stbi_write_jpg("sub_25_2.jpg", 25, 25, 3, destarr, 100);

    if (rc != DDS_RETCODE_OK) {
        fprintf(stderr, "! Unable to return loan, error %d\n", rc);
    }
	//while (sub < 1875) {
		//fprintf(subfp, "%d", destarr[sub]);
		//_putw( destarr[sub],subfp);
		//sub++;
	//}
